...
const Slide3Action = () => (
  <SlideLayout bgImage={slide3Bg} overlayStrong>
    ...
    >
      ...
      <motion.p className="text-xl text-primary-foreground/60 mb-14 font-light max-w-xl" variants={item}>
        Selecciona el medio de comunicación usado para contactar al deudor.
      </motion.p>
      <motion.div className="grid grid-cols-3 gap-5 max-w-3xl" variants={container}>
        {channels.map((ch) => (
          <motion.div
            key={ch.label}
            className="card-glass-strong rounded-xl p-6 flex items-center gap-5 cursor-default"
            variants={item}
