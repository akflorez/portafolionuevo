...
const Slide7Reminder = () => (
  <SlideLayout bgImage={slide7Bg}>
    <motion.div className="flex-1 flex flex-col items-center justify-center px-20 text-center" variants={staggerContainer} initial="hidden" animate="show">
      <motion.div className="w-24 h-24 rounded-3xl bg-primary/20 flex items-center justify-center mb-8 glow-green" variants={fadeUp}>
        <ShieldCheck className="w-12 h-12 text-primary" />
      </motion.div>
      <motion.h2 className="text-5xl font-extrabold text-primary-foreground mb-6" variants={fadeUp}>Recuerda</motion.h2>
      <motion.p className="text-2xl text-primary-foreground/70 max-w-3xl mb-14 font-light leading-relaxed" variants={fadeUp}>
      ...
      <motion.div className="flex gap-6" variants={staggerContainer}>
        {reminders.map((r) => (
          <motion.div key={r.label} className="card-glass rounded-xl p-5 flex flex-col items-center gap-3 min-w-[120px]" variants={fadeUp}>
